...
class RegistrationForm(FlaskForm):
    ...
    password = PasswordField('Tvoja lozinka', validators=[DataRequired()])
    ...
    def validate_username(self, username):
        ...
        if user:
            raise ValidationError('Izabrano korisničko ime je zauzeto. Probaj sa nekim drugim. ')
    def validate_email(self, email):
        user = User.query.filter_by(email=email.data).first()
        if user:
            raise ValidationError('Izabrana email adresa je već u upotrebi. ')
...
class UpdateAccountForm(FlaskForm):
    ...
    picture = FileField('Ažuriraj sliku na profilu', validators=[FileAllowed(['jpg','png'])])
